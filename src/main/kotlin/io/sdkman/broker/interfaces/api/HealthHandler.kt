...
class HealthHandler(private val healthService: HealthService) {
     ...
     */
    suspend fun handle(call: ApplicationCall): Unit = healthService.checkHealth().fold(
     ...
     */
    private suspend fun handleError(call: ApplicationCall, error: DomainError) = when (error) {
        is DomainError.AppNotFound -> call.respond(
            HttpStatusCode.ServiceUnavailable,
            mapOf("status" to "DOWN", "reason" to error.message)
        )
        is DomainError.AppNotHealthy -> call.respond(
            HttpStatusCode.ServiceUnavailable,
            mapOf("status" to "DOWN", "reason" to error.message)
        )
